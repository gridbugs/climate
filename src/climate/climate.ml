...
let help_command_arg_parser () = Arg_parser.usage ~error:false ~message:None
...
module Command = struct
  ...
  (* Recursively replace all parsers in t with parsers which just print their
     ...
     commands. The resulting parsers are not intended to be "finalized", and
     thus lack help messages of their own. That is you can't run `git help
     commit --help`, and attempting to do so will print an error that hopefully
     instructs users to a more useful help command. *)
  let replace_with_usage t =
    match t with
    | Singleton { arg_parser; doc } ->
      let arg_parser = Arg_parser.to_usage arg_parser in
      Singleton { arg_parser; doc }
    | x ->
      (* TODO *)
      x
      ...
      | Internal Help ->
        let siblings =
          List.map siblings ~f:(fun { info; command; help } ->
            { info; command = replace_with_usage command; help })
        in
        let child_subcommands =
          List.filter_map siblings ~f:(fun { info; command; help = _ } ->
            if info.hidden
            then None
            else
              Some
          ...
          `Finalized
            (help_command_arg_parser ()
             |> Arg_parser.Private.finalize
                  ...
                  ~help_only_doc:
                    (Some
                       "Subcommand help. Pass the name of a subcommand (see below) for \
                        info."))
        in
